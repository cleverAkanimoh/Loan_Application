...
class MyFrame(Frame):
    ...
    def initUI(self):
        self.parent.title("Clever Loan Calculator")
        self.style = Style()
        self.style.theme_use("default") # default 
 
        self.pack(fill=BOTH, expand=1) 

        label=Label(self,text="Welcome \t Calculation Made Easy!",
font="Georgia 11")
        label.pack(padx=15,pady=10,fill=BOTH)

        xpos = 60
        ypos = 50
        xpos2 = xpos+100

        l1 = Label(self, text="Sample Amount :", foreground = "#ff0000", background = "light blue",
font = "Arial 9")
# Arial 12 bold italic
        ...
        l2 = Label(self, text="Rate(in %) :", foreground = "#ff0000", background = "light blue",
font = "Arial 9")
# Arial 12 bold italic
        l2.place(x=xpos, y=ypos)
        self.txtRate = Entry()
        self.txtRate.place(x=xpos2, y = ypos, width=70)
        ypos += 30
...
# Arial 12 bold italic
        ...
        self.txtDuration.place(x=xpos2, y = ypos, width=70) 
        ypos += 30
...
# Arial 12 bold italic
        ...
        self.txtMonthlyPayment.place(x=xpos2, y = ypos, width=70) 
        ypos += 30
        
        l5 = Label(self, text="Total Payments :", foreground = "#ff0000", background = "yellow", font = "Arial 9")
#Arial 12 bold italic
        l5.place(x=xpos, y=ypos)
        self.txtTotalPayment = Entry();self.txtTotalPayment.configure(state="readonly")
        self.txtTotalPayment.place(x=xpos2, y = ypos, width=70) 
        ypos += 30

        style = Style()
        style.configure("Exit.TButton", foreground="orange", background="red")  #T.Checkbutton for checboxes
        style.configure("MainButton.TButton", foreground="yellow", background="green")
    ...
    def exitButtonClick(self):
        if (messagebox.askokcancel("Close Application?","Click on OK to close?")):
            self.parent.destroy
